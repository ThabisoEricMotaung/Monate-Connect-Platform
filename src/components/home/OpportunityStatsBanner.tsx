...
async function getOpportunityStats(): Promise<OpportunityStats | null> {
  try {
    ...
    const [liveRes, closingWeekRes, new48Res, screenedTotalRes, screenedDoneRes] = await Promise.all([
      eligibleBase(),
      eligibleBase().lte("closing_date", in7DaysIso),
    ...
    for (const res of [liveRes, closingWeekRes, new48Res, screenedTotalRes, screenedDoneRes]) {
      if (res.error) {
        console.warn("Opportunity stats query failed:", res.error.message)
        return null
      }
    }
    ...
    console.warn("Opportunity stats query failed:", err)
    return null
  }
}
