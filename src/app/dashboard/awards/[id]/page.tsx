...
export default function AwardWorkspacePage() {
  ...
  const [supplierScore, setSupplierScore] = useState<number | null>(null)
  ...
  const loadAward = useCallback(async () => {
    ...
    try {
      ...
      if (supabase && loadedPo.supplier_id) {
        const canonicalScore = await getCanonicalSupplierSmartScore(loadedPo.supplier_id, supabase)
        setSupplierScore(canonicalScore?.result.score ?? null)
      } else {
        setSupplierScore(null)
      }
