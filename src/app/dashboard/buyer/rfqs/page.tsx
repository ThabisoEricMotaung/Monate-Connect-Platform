...
function statusBadgeClass(status: string | null): string {
  ...
  if (value === "open") return "border-emerald-500/25 bg-emerald-500/10 text-emerald-700"
  if (value === "evaluation") return "border-sky-500/25 bg-sky-500/10 text-sky-700"
  if (value === "awarded") return "border-accent/25 bg-accent/10 text-accent"
  if (value === "closed") return "border-panel bg-surface text-secondary"
...
function getDeadline(rfq: BuyerRFQ): string | null {
  return rfq.deadline ?? rfq.closing_date
}
...
export default function BuyerRfqsPage() {
  ...
  useEffect(() => {
    ...
    async function loadRFQs() {
      ...
      if (!user) {
        setErrorMessage("Sign in again to load your RFQs.")
        setLoading(false)
        return
      }
      ...
      const { data, error } = await supabase
        .from("rfqs")
        .select("id, title, category, province, region, deadline, closing_date, status, quote_count, created_at")
    ...
    <div>
      <div className="mb-8 border-b border-panel pb-6">
        ...
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-heading">My RFQs</h1>
      ...
      <div className="mb-5 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-md border border-panel bg-card p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-secondary">Quotes received</p>
          <p className="mt-2 text-3xl font-bold text-heading">
            {loading ? "-" : rfqs.reduce((total, rfq) => total + (rfq.quote_count ?? 0), 0)}
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-panel bg-card text-left text-xs font-semibold uppercase tracking-[0.18em] text-secondary">
                ...
                <th className="hidden px-5 py-3 lg:table-cell">Deadline</th>
                <th className="hidden px-5 py-3 sm:table-cell">Quotes</th>
            ...
            <tbody className="divide-y divide-panel bg-card">
              {rfqs.map((rfq) => (
                <tr key={rfq.id} className="transition hover:bg-surface">
                  ...
                  <td className="hidden px-5 py-4 text-secondary lg:table-cell">
                    {formatDate(getDeadline(rfq))}
                  </td>
                  <td className="hidden px-5 py-4 font-semibold text-heading sm:table-cell">
                    {rfq.quote_count ?? 0}
                  </td>
