...
// Public, no-login-required unsubscribe link clicked from the weekly public
// opportunities digest. Mirrors src/app/api/unsubscribe/weekly-digest —
// identified by the subscriber row's own uuid (effectively unguessable)
// rather than a session, since the person clicking it is reading an email.
...
export async function GET(request: Request) {
  ...
  if (error) {
    console.error("Opportunity digest unsubscribe failed:", error)
    return new NextResponse(htmlPage("Something went wrong", "We couldn't process this right now. Please try again shortly."), {
      status: 500,
      headers: { "Content-Type": "text/html" },
    })
  ...
  return new NextResponse(
    htmlPage(
      "You're unsubscribed",
      "You won't receive the weekly opportunities email anymore.",
    ),
